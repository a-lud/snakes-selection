...
match_19 <- unique(marine_synonyms$symbol)[unique(marine_synonyms$symbol) %in% unique(gene_synonyms$symbol)]
...
marine_synonym <- marine_synonyms %>% filter(! symbol %in% match_19)
lit_synonym <- gene_synonyms %>% filter( ! gene_synonyms$synonym %in% match_19 )

overlap_synonym <- inner_join(marine_synonym, lit_synonym, by = 'synonym') %>% arrange(symbol.x)

write_csv(
  x = overlap_synonym_379,
  file = here(
...
overlap_379 <- gene_synonyms %>% filter(symbol %in% match_19) %>%
